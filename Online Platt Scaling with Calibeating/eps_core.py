...
def test_and_update(storage_list, function, y, pred_probs):
    storage_list.append(function(y, pred_probs, fixed=True, n_bins=5))
...
def run_exp(dataset="churn", seed=0, design=0, tune=False):
    ...
    ############################################
    _, fixed_platt_a, fixed_platt_b = calibration.fit_platt_scaling_parameters(utils.logit(rf_preds[:n_val]), y_test[:n_val])
    ...
    for n_calib_points in N_calib_points:
        ...
        test_and_update(SHARPNESS_base, assessment.sharpness, y_test[n_val:n_calib_points], rf_preds[n_val:n_calib_points])

        ############################################
        ### Fixed batch Platt scaling
        ############################################
        test_and_update(ECE_recalib_platt_const, assessment.ece, y_test[n_val:n_calib_points], pred_probs_fixed_platt[:(n_calib_points-n_val)])
        test_and_update(SHARPNESS_recalib_platt_const, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_fixed_platt[:(n_calib_points-n_val)])
        ...
        ############################################    
        test_and_update(ECE_recalib_platt_continuous_beat, assessment.ece, y_test[n_val:n_calib_points], pred_probs_platt_beat[n_val:n_calib_points])
        test_and_update(SHARPNESS_recalib_platt_continuous_beat, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_platt_beat[n_val:n_calib_points])
    
        ############################################
        ### Online Platt Scaling (OPS) + calibeating
        ############################################    
        test_and_update(ECE_recalib_platt_continuous_calibeat, assessment.ece, y_test[n_val:n_calib_points], pred_probs_platt_calibeat[n_val:n_calib_points])
        test_and_update(SHARPNESS_recalib_platt_continuous_calibeat, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_platt_calibeat[n_val:n_calib_points])
