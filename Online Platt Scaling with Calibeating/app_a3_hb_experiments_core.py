...
def run_exp(dataset="churn", seed=0, design=0, tune=False):
    ...
    ############################################
    ### Windowed histogram binning (WHB)
    ############################################
    N_window_indices = np.arange(n_val+twice_spacing, rf_preds.size+twice_spacing, spacing).astype('int')
    pred_probs_hb = np.zeros((rf_preds.size-n_val))

    for n_calib_points in N_window_indices:
        hb = calibration.HB_binary(n_bins = 20)
        hb.fit(rf_preds[:(n_calib_points-twice_spacing)], y_test[:(n_calib_points-twice_spacing)])
        pred_probs_hb[(n_calib_points-(n_val+twice_spacing)):(n_calib_points-(n_val+spacing))] = hb.predict_proba(
    ...
    pred_probs_platt_track = np.zeros((pred_probs_platt.size))
    for i in range(pred_probs_platt_track.size):
        bin_i = pred_probs_platt_digitized[i]
        pred_probs_platt_track[i] = bin_means[bin_i]
    ...
    pred_probs_beta_track = np.zeros((pred_probs_beta.size))
    for i in range(pred_probs_beta_track.size):
        bin_i = pred_probs_beta_digitized[i]
        pred_probs_beta_track[i] = bin_means[bin_i]
        bin_means[bin_i] = ((bin_num[bin_i]*bin_means[bin_i]) + y_test[i]
                            )/(bin_num[bin_i]+1)
        bin_num[bin_i] += 1
    ...
    bin_means = np.linspace(1.0/n_bins, 1.0, n_bins) - 0.05/n_bins
    bin_num = np.ones((n_bins))
    pred_probs_hb_digitized = utils.digitize_predictions_index(pred_probs_hb, n_bins)
    pred_probs_hb_track = np.zeros((pred_probs_hb.size))
    for i in range(pred_probs_hb.size):
        bin_i = pred_probs_hb_digitized[i]
        pred_probs_hb_track[i] = bin_means[bin_i]
        bin_means[bin_i] = ((bin_num[bin_i]*bin_means[bin_i]) + y_test[n_val+i]
                            )/(bin_num[bin_i]+1)
        bin_num[bin_i] += 1
    ...
    ECE_recalib_beta = []
    ECE_recalib_hb = []
    ECE_recalib_platt_track = []
    ECE_recalib_beta_track = []
    ECE_recalib_hb_track = []

    SHARPNESS_recalib_platt = []
    SHARPNESS_recalib_beta = []
    SHARPNESS_recalib_hb = []
    SHARPNESS_recalib_platt_track = []
    SHARPNESS_recalib_beta_track = []
    SHARPNESS_recalib_hb_track = []
    ...
    for n_calib_points in N_calib_points:
        ...
        ### Online Platt Scaling (OPS)
        ############################################
        test_and_update(ECE_recalib_platt, assessment.ece, y_test[n_val:n_calib_points], pred_probs_platt[n_val:n_calib_points])
        test_and_update(SHARPNESS_recalib_platt, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_platt[n_val:n_calib_points])
    
        ############################################
        ### Online beta Scaling (OBS)
        ############################################
        test_and_update(ECE_recalib_beta, assessment.ece, y_test[n_val:n_calib_points], pred_probs_beta[n_val:n_calib_points])
        test_and_update(SHARPNESS_recalib_beta, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_beta[n_val:n_calib_points])
    
        ############################################
        ### Windowed Histogram Binning (WHB)
        ############################################
        test_and_update(ECE_recalib_hb, assessment.ece, y_test[n_val:n_calib_points], pred_probs_hb[:(n_calib_points-n_val)])
        test_and_update(SHARPNESS_recalib_hb, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_hb[:(n_calib_points-n_val)])
        
        ############################################
        ### Online Platt Scaling (OPS) + tracking
        ############################################
        test_and_update(ECE_recalib_platt_track, assessment.ece, y_test[n_val:n_calib_points], pred_probs_platt_track[n_val:n_calib_points])
        test_and_update(SHARPNESS_recalib_platt_track, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_platt_track[n_val:n_calib_points])
    
        ############################################
        ### Online beta Scaling (OBS) + tracking
        ############################################
        test_and_update(ECE_recalib_beta_track, assessment.ece, y_test[n_val:n_calib_points], pred_probs_beta_track[n_val:n_calib_points])
        test_and_update(SHARPNESS_recalib_beta_track, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_beta_track[n_val:n_calib_points])
    
        ############################################
        ### Windowed Histogram Binning (WHB) + tracking
        ############################################
        test_and_update(ECE_recalib_hb_track, assessment.ece, y_test[n_val:n_calib_points], pred_probs_hb_track[:(n_calib_points-n_val)])
        test_and_update(SHARPNESS_recalib_hb_track, assessment.sharpness, y_test[n_val:n_calib_points], pred_probs_hb_track[:(n_calib_points-n_val)])
        

    return ECE_recalib_platt, \
        ECE_recalib_beta,\
        ECE_recalib_hb, \
        ECE_recalib_platt_track, \
        ECE_recalib_beta_track,\
